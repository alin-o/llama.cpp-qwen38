...
- Stage advancement is automatic: when a stage run finishes, do NOT call `update_task` with a `status` pointing at the next stage - the board advances the card by itself. Just finish the stage's work (code, tests, notes, commits) and end the run. If a later stage must be skipped, set the task's `skip_stages` field instead of moving the card manually.
