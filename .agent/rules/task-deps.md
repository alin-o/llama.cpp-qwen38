...
- Use one-shot stage comments for ordinary handoffs and transient run instructions. Use persistent stage comments for stable requirements that must survive retries or reach future agents, including scope boundaries, accepted evidence, ownership decisions, and testing or review gates. Attach the guidance separately to every stage that must enforce it; a resume message or comment on the current stage does not reliably guide later stages. Persistent comments are also appropriate after the same root failure recurs in a stage.
