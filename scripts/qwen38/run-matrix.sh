...
cleanup() {
    "${COMPOSE[@]}" stop llama-server >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
selected upstream-q8q5-base && run_one upstream llama-server "${QWEN38_UPSTREAM_PORT:-8882}" upstream-q8q5-base q8q5 off
selected upstream-q8q5-mtp && run_one upstream llama-server "${QWEN38_UPSTREAM_PORT:-8882}" upstream-q8q5-mtp q8q5 mtp
selected upstream-iq4nl-base && run_one upstream llama-server "${QWEN38_UPSTREAM_PORT:-8882}" upstream-iq4nl-base iq4nl off
selected upstream-iq4nl-mtp && run_one upstream llama-server "${QWEN38_UPSTREAM_PORT:-8882}" upstream-iq4nl-mtp iq4nl mtp
