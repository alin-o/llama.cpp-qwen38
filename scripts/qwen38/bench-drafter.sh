...
# Restarts the llama-server container per config (port 8881) and runs the
...
start_server() { # $1=PROFILE $2=SPEC, rest: extra env
    ...
    log "starting llama-server (PROFILE=$profile SPEC=$spec, env: $*)"
    env "$@" CACHE_PROFILE="$profile" SPEC="$spec" QWEN38_UPSTREAM_PORT="$PORT" NP=1 \
        ...
        up -d --build llama-server 2>&1 | tee -a "$LOG_FILE"
    wait_health
}
...
stop_server() {
    ...
    docker compose -f "$COMPOSE_FILE" --profile upstream stop llama-server >>"$LOG_FILE" 2>&1
    sleep 15
}
