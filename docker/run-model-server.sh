#!/usr/bin/env bash
# Generic llama-server launcher for every model configuration.
#
# Required: MODEL_GGUF (in-container path, e.g. /models/<file>.gguf).
# Optional: CTX NP CACHE_PROFILE CACHE_REUSE THREADS THREADS_HTTP HOST PORT
#           SLOT_PROMPT_CACHE_THRESHOLD CACHE_RAM_MIB SLOT_SAVE_PATH CHAT_TEMPLATE_FILE
...
#           SPEC=dspark   run the draft model from DRAFT_GGUF
...
# blk.<n>.nextn.*). Detect it so --spec-type draft-mtp can run without a
# separate --spec-draft-model.
...
elif [[ "${SPEC:-off}" == "dspark" ]]; then
    [[ -n "${DRAFT_GGUF:-}" && -f "${DRAFT_GGUF}" ]] \
        || { echo "error: SPEC=dspark needs DRAFT_GGUF" >&2; exit 1; }
    ARGS+=(
...
echo "model: engine=generic profile=${PROFILE} spec=${SPEC:-off} ctx=${CTX:-32768} np=${NP:-1} kv=${KV_MODE} cache_ram_mib=${CACHE_RAM_MIB:-16384} model=${MODEL} prompt_log=${PROMPT_LOG_DIR:-off}" >&2
exec "$SERVER" "${ARGS[@]}" "$@"
