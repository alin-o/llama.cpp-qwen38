#!/usr/bin/env bash
#
# Single launcher for the llama-server stack.
...
#   ./run.sh ornith1.5      # a manually downloaded model
...
# MODEL_ENTRYPOINT=/path ./run.sh <model>  override the default launcher
#
...
#
# One model at a time: starting a model recreates the same llama-server
...
MANAGED_VARS="MODEL_GGUF MTP_GGUF MMPROJ_GGUF DRAFT_GGUF MODEL_ENTRYPOINT CACHE_PROFILE \
CHAT_TEMPLATE_FILE REASONING_FORMAT PROMPT_LOG_DIR \
SPEC CTX DRAFT_MAX DRAFT_CACHE_K DRAFT_CACHE_V NP CACHE_REUSE \
SLOT_PROMPT_CACHE_THRESHOLD CACHE_RAM_MIB QWEN38_UPSTREAM_PORT MODELS_DIR \
...
cmd_start() {
    ...
    echo
    local spec_default="off"
    local model_entrypoint="${MODEL_ENTRYPOINT:-/run-model-server.sh}"
    echo "============================================================"
