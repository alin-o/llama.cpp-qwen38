...
static ggml_cuda_graph_key ggml_cuda_graph_get_key(ggml_cgraph * cgraph) {
    ...
    const auto hash_value = [&topology_hash](uint64_t value) {
        topology_hash ^= value;
        topology_hash *= 1099511628211ULL;
    };
    hash_value(cgraph->n_nodes);
    hash_value(cgraph->n_leafs);
    for (int i = 0; i < cgraph->n_nodes; ++i) {
        const ggml_tensor * node = cgraph->nodes[i];
        hash_value(node->op);
        hash_value(node->type);
        for (int dim = 0; dim < GGML_MAX_DIMS; ++dim) {
            hash_value(node->ne[dim]);
            hash_value(node->nb[dim]);
        }
    }
    return { cgraph->nodes[0], topology_hash };
}
