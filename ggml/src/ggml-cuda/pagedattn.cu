...
__global__ void paged_attention_write_kernel(const float * __restrict__ k_new,  // [batch_size, n_heads_kv, head_dim]
    ...
    for (int i = 0; i < num_tokens; ++i) {
        ...
        const size_t v_cache_idx = (size_t) block_id * stride_block + (size_t) head_idx * stride_head +
                                   (size_t) token_in_block * stride_token + tid;
        ...
        k_cache[k_cache_idx] = __float2half(k_new[input_off]);
        v_cache[v_cache_idx] = __float2half(v_new[input_off]);
    }
}
    ...
    for (int i = 0; i < num_new_tokens; i++) {
        ...
        for (int bid = 0; bid < num_blocks; bid++) {
            ...
            for (int token = start_token; token < end_token; ++token) {
                ...
                const size_t v_idx = tid + token_in_block * stride_token + kv_head_idx * stride_head +
                                     physical_block * stride_block;

                float k_val = __half2float(k_cache[k_idx]);
...
__global__ void paged_attention_prefill_kernel(const float * __restrict__ q,
    ...
    for (int bid = 0; bid <= q_pos / block_size; ++bid) {
        ...
        for (int token = start_token; token < end_token; ++token) {
            ...
            const size_t v_base = token_in_block * stride_token + kv_head_idx * stride_head + physical_block * stride_block;

            float qk = 0.0f;
    for (int d = lane; d < head_dim; d += 32) {
                qk += q[(size_t) token_idx * n_heads * head_dim + (size_t) head_idx * head_dim + d] *
                      __half2float(k_cache[k_base + d]);
            }
            qk *= scale;
            for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    for (int d = lane; d < head_dim; d += 32) {
                const int i = d / 32;
                acc[i] = acc[i] * exp_old + exp_new * __half2float(v_cache[v_base + d]);
            }
            qk_max = qk_max_new;
        }
    }
...
void ggml_cuda_op_paged_attn(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const size_t stride_head  = k_cache->nb[2] / sizeof(half);
    const size_t stride_block = k_cache->nb[3] / sizeof(half);
    ...
    paged_attention_write_kernel<<<dim3(n_heads_kv, n_seq), dim3(head_dim), 0, ctx.stream()>>>(
        (const float *) k_new->data, (const float *) v_new->data, (half *) k_cache->data, (half *) v_cache->data,
    ...
    paged_attention_decode_kernel<<<dim3(n_heads, n_seq), dim3(head_dim), smem_bytes, ctx.stream()>>>(
        (const float *) q->data, (const half *) k_cache->data, (const half *) v_cache->data, (const int *) block_table->data,
