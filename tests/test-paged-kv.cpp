...
TEST(test_scheduler_reports_cache_capacity_and_retention) {
    auto fixture = make_fixture(/*n_ctx=*/128, /*block_size=*/16, /*n_batch=*/64,
                                /*n_gpu_blocks=*/8, /*n_cpu_blocks=*/2);
    llama_paged_cache_stats stats = fixture.sched->get_cache_stats();
    EXPECT_EQ(stats.block_size, 16u);
    EXPECT_EQ(stats.n_gpu_blocks, 8u);
    EXPECT_EQ(stats.n_gpu_blocks_free, 8u);
    EXPECT_EQ(stats.n_cpu_blocks, 2u);
    EXPECT_EQ(stats.n_cpu_blocks_free, 2u);
    ...
    stats = fixture.sched->get_cache_stats();
    EXPECT_EQ(stats.n_gpu_blocks_free, 5u);
    EXPECT_EQ(stats.n_cpu_blocks_free, 2u);
    EXPECT_EQ(stats.n_retained, 1u);

    EXPECT_TRUE(fixture.sched->queue_request(make_group(/*id=*/0, /*n_prompt=*/35), /*n_past=*/18));
    stats = fixture.sched->get_cache_stats();
    EXPECT_EQ(stats.n_gpu_blocks_free, 6u);
    EXPECT_EQ(stats.n_retained, 0u);
...
TEST(test_cumulative_checkpoint_cross_slot_sharing_and_cow) {
    ...
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().hits, 0u);
    EXPECT_TRUE(fixture.sched->commit_cached_request(1));
    EXPECT_TRUE(fixture.sched->commit_cached_request(2));
    EXPECT_FALSE(fixture.sched->commit_cached_request(2));
    ...
    llama_sequence_group rejected = make_group(/*id=*/5, /*n_prompt=*/24);
    for (uint32_t i = 0; i < 18; ++i) {
        rejected.logical_seq[i] = i;
    }
    const llama_checkpoint_metrics before_rejected_restore = fixture.sched->get_checkpoint_metrics();
    EXPECT_TRUE(fixture.sched->queue_request_cached(
        std::move(rejected), "test-fingerprint", nullptr, nullptr));
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().hits, before_rejected_restore.hits);
    fixture.sched->remove_request(5);
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().hits, before_rejected_restore.hits);
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().hit_tokens, before_rejected_restore.hit_tokens);
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().suffix_tokens, before_rejected_restore.suffix_tokens);
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().restore_successes,
              before_rejected_restore.restore_successes);
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().restore_failures,
              before_rejected_restore.restore_failures + 1);
    fixture.sched->remove_request(0);
    ...
    EXPECT_TRUE(fixture.sched->commit_cached_request(3));
    EXPECT_TRUE(fixture.sched->commit_cached_request(4));
...
TEST(test_cumulative_checkpoint_overlapping_lineage_and_builder_cancellation) {
    ...
    EXPECT_TRUE(concurrent.sched->commit_cached_request(1));
    EXPECT_EQ(used, 32u);
...
TEST(test_cumulative_checkpoint_publication_cow_rollback) {
    auto fixture = make_fixture(/*n_ctx=*/128, /*block_size=*/16, /*n_batch=*/64,
                                /*n_gpu_blocks=*/8, /*n_cpu_blocks=*/2);
    test_paged_batch_policy policy;
    policy.split_request_id = 0;
    ...
    llama_sequence_group source = make_group(/*id=*/0, /*n_prompt=*/24);
    for (uint32_t i = 0; i < source.n_prompt; ++i) {
        source.logical_seq[i] = i;
    }
    EXPECT_TRUE(fixture.sched->queue_request(std::move(source)));
    ...
    payload.draft = { 2 };
    payload.speculative = { 3 };
    const llama_block_ids original_blocks = fixture.sched->request_block_ids(0);
    const uint32_t original_free_blocks = fixture.kv->get_num_free_gpu_blocks();
    EXPECT_EQ(original_blocks.size(), 2u);

    fixture.sched->set_checkpoint_quotas_for_test(8, 1);
    EXPECT_FALSE(fixture.sched->publish_checkpoint(0, 18, "cow-quota", payload));
    EXPECT_TRUE(fixture.sched->request_block_ids(0) == original_blocks);
    EXPECT_EQ(fixture.kv->get_num_free_gpu_blocks(), original_free_blocks);
    EXPECT_EQ(fixture.kv->get_block_ref_count(original_blocks[0]), 1u);
    EXPECT_EQ(fixture.kv->get_block_ref_count(original_blocks[1]), 1u);
    fixture.sched->set_checkpoint_quotas_for_test(8, 1024 * 1024);
    ...
    for (size_t i = 0; i < sizeof(faults) / sizeof(faults[0]); ++i) {
        EXPECT_FALSE(fixture.sched->publish_checkpoint(
            0, 18, "cow-component-" + std::to_string(i), payload, nullptr, faults[i]));
        EXPECT_TRUE(fixture.sched->request_block_ids(0) == original_blocks);
        EXPECT_EQ(fixture.kv->get_num_free_gpu_blocks(), original_free_blocks);
        EXPECT_EQ(fixture.kv->get_block_ref_count(original_blocks[1]), 1u);
    }

    fixture.sched->set_checkpoint_cow_gate_for_test(true);
    bool cancelled_published = true;
    std::thread builder([&] {
        cancelled_published = fixture.sched->publish_checkpoint(
            0, 18, "cow-cancelled", payload);
    });
    EXPECT_TRUE(fixture.sched->wait_for_checkpoint_cow_gate_for_test(1000));
    EXPECT_TRUE(fixture.sched->request_block_ids(0) != original_blocks);
    fixture.sched->remove_request(0);
    fixture.sched->set_checkpoint_cow_gate_for_test(false);
    builder.join();
    EXPECT_FALSE(cancelled_published);
    EXPECT_EQ(fixture.sched->get_checkpoint_metrics().records, 0u);
    EXPECT_EQ(fixture.kv->get_num_free_gpu_blocks(), 8u);
    llama_batch_free(batch);
}
...
TEST(test_paged_attention_head_mapping_and_dispatch_selection) {
    ...
    for (const ggml_type cache_type : { GGML_TYPE_Q8_0, GGML_TYPE_TURBO3_0, GGML_TYPE_TURBO4_0 }) {
        EXPECT_TRUE(ggml_paged_attn_select_cuda_prefill_row_cache(
            GGML_PAGED_ATTN_CONTEXT_4K, 256, 16, 2, 1, 1023, cache_type, cache_type, ada));
    }
    for (const bool fallback : {
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_16K, 256, 16, 2, 1, 1023,
                GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, ada),
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_4K, 128, 16, 2, 1, 1023,
                GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, ada),
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_4K, 256, 24, 4, 1, 1023,
                GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, ada),
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_4K, 256, 16, 2, 2, 1023,
                GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, ada),
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_4K, 256, 16, 2, 1, 1024,
                GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, ada),
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_4K, 256, 16, 2, 1, 1023,
                GGML_TYPE_TURBO3_0, GGML_TYPE_TURBO4_0, ada),
            ggml_paged_attn_select_cuda_prefill_row_cache(
                GGML_PAGED_ATTN_CONTEXT_4K, 256, 16, 2, 1, 1023,
                GGML_TYPE_F16, GGML_TYPE_F16, ada),
        }) {
    ...
    EXPECT_TRUE(ggml_paged_attn_cuda_turbo_token_vec_supported(
        GGML_PAGED_ATTN_CONTEXT_4K, 256, 32, 4, 1, 1,
        GGML_TYPE_TURBO3_0, GGML_TYPE_TURBO3_0, ada));
    EXPECT_TRUE(ggml_paged_attn_cuda_turbo_token_vec_supported(
        GGML_PAGED_ATTN_CONTEXT_16K, 256, 24, 4, 8, 8,
        GGML_TYPE_TURBO4_0, GGML_TYPE_TURBO4_0, ada));
    for (const bool fallback : {
    ...
    for (const ggml_paged_attn_cuda_device_caps unsupported : {
            ...
            ggml_paged_attn_cuda_device_caps{ 890, 128, 32, 64, 48 * 1024, true, true },
            ggml_paged_attn_cuda_device_caps{ 890, 128, 32, 1024, 4 * 1024, true, true },
        ...
        EXPECT_TRUE(fallback.n_warps == 32 && fallback.n_partitions == 1 && fallback.n_q_heads == 1);
        EXPECT_FALSE(ggml_paged_attn_cuda_turbo_token_vec_supported(
            GGML_PAGED_ATTN_CONTEXT_4K, 256, 32, 4, 1, 1,
            GGML_TYPE_TURBO3_0, GGML_TYPE_TURBO3_0, unsupported));
        EXPECT_FALSE(ggml_paged_attn_select_cuda_prefill_row_cache(
            GGML_PAGED_ATTN_CONTEXT_4K, 256, 16, 2, 1, 1023,
            GGML_TYPE_Q8_0, GGML_TYPE_Q8_0, unsupported));
    }
}
...
static void check_paged_attention_turbo_decode_case(
    ...
    ggml_paged_attn_turbo_token_vec_launch_count_reset();
    ggml_paged_attn_turbo_combined_write_launch_count_reset();
    const paged_decode_result actual = run_paged_attention_decode_case(
        cuda_backend, n_heads, n_heads_kv, context_length, n_sequences, block_size,
        nullptr, nullptr, 1, false, nullptr, cache_type);
    EXPECT_TRUE(ggml_paged_attn_turbo_decode_launch_count() > 0);
    EXPECT_TRUE(ggml_paged_attn_turbo_token_vec_launch_count() > 0);
    EXPECT_TRUE(ggml_paged_attn_turbo_combined_write_launch_count() > 0);
    EXPECT_TRUE(actual.k_cache == reference.k_cache);
    EXPECT_TRUE(actual.v_cache == reference.v_cache);
    ...
    fprintf(stderr, " %s gqa=%d depth=%d sequences=%d token-vec=%llu error=%g ",
        ggml_type_name(cache_type), n_heads / n_heads_kv, context_length, n_sequences,
        ggml_paged_attn_turbo_token_vec_launch_count(), error);
...
TEST(test_paged_attention_turbo_decode_boundaries_and_dispatch) {
    ...
    for (const ggml_type cache_type : { GGML_TYPE_TURBO3_0, GGML_TYPE_TURBO4_0 }) {
        for (int context_length : { 15, 16, 17, 33, 127, 128, 129 }) {
        ...
        check_paged_attention_turbo_decode_case(
            cpu_backend, cuda_backend, cache_type, 32, 4, 129, 2, 16);

        setenv("GGML_CUDA_PAGED_TURBO_TOKEN_VEC", "0", 1);
        ggml_paged_attn_turbo_decode_launch_count_reset();
        ggml_paged_attn_turbo_token_vec_launch_count_reset();
        ggml_paged_attn_turbo_combined_write_launch_count_reset();
        const paged_decode_result fallback = run_paged_attention_decode_case(
            cuda_backend, 32, 4, 129, 1, 16, nullptr, nullptr, 1, false, nullptr, cache_type);
        unsetenv("GGML_CUDA_PAGED_TURBO_TOKEN_VEC");
        EXPECT_TRUE(ggml_paged_attn_turbo_decode_launch_count() > 0);
        EXPECT_TRUE(ggml_paged_attn_turbo_token_vec_launch_count() == 0);
        EXPECT_TRUE(ggml_paged_attn_turbo_combined_write_launch_count() > 0);
        const paged_decode_result reference = run_paged_attention_decode_case(
            cpu_backend, 32, 4, 129, 1, 16, nullptr, nullptr, 1, false, nullptr, cache_type);
        EXPECT_TRUE(fallback.k_cache == reference.k_cache);
        EXPECT_TRUE(fallback.v_cache == reference.v_cache);
        EXPECT_TRUE(paged_decode_relative_squared_error(fallback.output, reference.output) < 2e-4);
    }
...
TEST(test_paged_attention_turbo_graph_replay_remap) {
    ggml_backend_t cpu_backend = ggml_backend_init_by_type(GGML_BACKEND_DEVICE_TYPE_CPU, nullptr);
    ggml_backend_t cuda_backend = ggml_backend_init_by_type(GGML_BACKEND_DEVICE_TYPE_GPU, nullptr);
    EXPECT_TRUE(cpu_backend != nullptr);
    EXPECT_TRUE(cuda_backend != nullptr);

    const std::vector<int32_t> initial_lens = { 1024, 513, 257, 129 };
    const std::vector<int32_t> remapped_lens = { 1023, 769, 385, 130 };
    for (const ggml_type cache_type : { GGML_TYPE_TURBO3_0, GGML_TYPE_TURBO4_0 }) {
        const paged_decode_result reference = run_paged_attention_decode_case(
            cpu_backend, 32, 4, 1024, 4, 16, &initial_lens, &remapped_lens,
            2, true, nullptr, cache_type);
        ggml_paged_attn_turbo_decode_launch_count_reset();
        ggml_paged_attn_turbo_token_vec_launch_count_reset();
        ggml_paged_attn_turbo_combined_write_launch_count_reset();
        const paged_decode_result replayed = run_paged_attention_decode_case(
            cuda_backend, 32, 4, 1024, 4, 16, &initial_lens, &remapped_lens,
            4, true, nullptr, cache_type);
        EXPECT_TRUE(ggml_paged_attn_turbo_decode_launch_count() == 2);
        EXPECT_TRUE(ggml_paged_attn_turbo_token_vec_launch_count() == 2);
        EXPECT_TRUE(ggml_paged_attn_turbo_combined_write_launch_count() == 2);
        EXPECT_TRUE(replayed.k_cache == reference.k_cache);
        EXPECT_TRUE(replayed.v_cache == reference.v_cache);
        EXPECT_TRUE(paged_decode_relative_squared_error(replayed.output, reference.output) < 2e-4);
    }
...
int main(int argc, char ** argv) {
    ...
    if (argc > 1 && strcmp(argv[1], "--paged-graph-test") == 0) {
        ...
        RUN(test_paged_attention_turbo_graph_replay_remap);
        return 0;
    }
    ...
    RUN(test_cumulative_checkpoint_publication_cow_rollback);
